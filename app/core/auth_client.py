# finance-service/app/core/auth_client.py
import httpx
...
async def get_current_finance_user(current_user: dict = Depends(get_current_active_user)):
    """
    ...
    if current_user.get("role") not in ["admin", "finance"]:
        raise HTTPException(
            ...
            detail="Insufficient permissions. Finance role required."
        )
    return current_user
